...
import org.third.orm.hibernate3.core.hello.persistence.ErrorEvent;
import org.third.orm.hibernate3.core.hello.persistence.Event;
import org.third.orm.hibernate3.core.hello.persistence.Person;
...
//        mgr.testQuery();
    ...
    private void testQuery() {
        ...
        query = session
                .createQuery("select p.id, p.age, p.userName.firstName, p.userName.lastName from Person p");
        for (Object p : query.list()) {
        ...
        List<ErrorEvent> errorEvents = new ArrayList<ErrorEvent>();
        while (rs.next()) {
            System.out.println(rs.getRowNumber());
            // rs.scroll(rs.getRowNumber());//0--1--2--...
            errorEvents.add((ErrorEvent) rs.get(0));
        }
     ...
     void testNativeSQLQuery() {
        ...
        List persons = session.createSQLQuery(
                "SELECT person1.* FROM H3_PERSON person1 WHERE ROWNUM<10").list();
    ...
    public void testUpdate() {
        ...
        Query update = session.createQuery("Update Magazine set isbn = :isbn where person.id=100")
                .setString("isbn", "059-9716-649");
